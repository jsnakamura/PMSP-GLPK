...
void PmspInstance::writeFile(const char* fname) const {
   ...
   fid << "param num_mach := " << m_numMachines << ';' << '\n' << "param num_jobs := " << m_numJobs << ';' << "\n\n";

   fid << "param P := " << '\n';

   for (int i = 0; i < m_numJobs; ++i) {
      for (int j = 0; j < m_numMachines; ++j) {
         fid << i + 1 << ' ' << j + 1 << ' ' << m_procTimes[i][j];
   ...
   for (int k = 0; k < m_numMachines; ++k) {
      for (int i = 0; i < m_numJobs; ++i) {
         for (int j = 0; j < m_numJobs; ++j) {
            fid << i + 1 << ' ' << j + 1 << ' ' << k + 1 << ' ' << m_setupTimes[i][j][k] << '\n';
         }
      }
   }
   fid << ";\n\n";
...
int main(int argc, char **argv) {
   ...
   inst.writeFile("/home/juliano/Área de Trabalho/Otim/pmsp.dat");
